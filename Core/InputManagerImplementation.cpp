		...
		if (l_Input.Exists())
		{
			for (int i = 0; i < l_Input.GetNumChildren(); ++i)
			{
				...
				else if (l_Element.GetName() == std::string("axis"))
				{
					...
					Axis axis = {};
					axis.name = l_Action.GetPszProperty("name");
					std::string type = l_Action.GetPszProperty("type", "MOUSE");
					axis.inputType = ParseInputType(type);
					std::string axis_str = l_Action.GetPszProperty("axis", "", false);

					switch (axis.inputType)
					{
					case MOUSE:
						axis.mouse.axis = ParseMouseAxis(axis_str);
...
CInputManagerImplementation::MouseAxis CInputManagerImplementation::ParseMouseAxis(const std::string& axis)
{
	if (axis == "X")
	{
		return MouseAxis::X;
...
CInputManagerImplementation::Action::MouseButton CInputManagerImplementation::ParseMouseButton(const std::string& button)
{
	if (button == "RIGHT")
	{
