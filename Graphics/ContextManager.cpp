...
void CContextManager::InitRasterizerStates()
{
...

	{
		...
		HRESULT l_HR = m_D3DDevice->CreateRasterizerState(&l_WireframeDesc, &m_RasterizerSates[RS_CULL_BACK]);
		assert(l_HR == S_OK);
	}

	{
		...
		HRESULT l_HR = m_D3DDevice->CreateRasterizerState(&l_WireframeDesc, &m_RasterizerSates[RS_CULL_FRONT]);
		assert(l_HR == S_OK);
	}
}
...
void CContextManager::InitDepthStencilStates()
{
...

	{
		...
		HRESULT l_HR = m_D3DDevice->CreateDepthStencilState(&l_desc, &m_DepthStencilStates[DSS_DEPTH_HIDE]);
		assert(l_HR == S_OK);
	}
